...
def _run(command: Command, output_format: str) -> None:
    code = CommandDispatcher().dispatch(
        command, out=sys.stdout, err=sys.stderr, output_format=output_format
    )
...
def generate(
    ...
    _run(GenerateCommand(project, quick=quick, timeout=timeout), output_format)
...
def optimize(
    ...
    _run(OptimizeCommand(project, solver=solver, seed=seed, timeout=timeout), output_format)
