...
def main() -> int:
    ...
    duties = sum(1 for c in con_deberes.courses if not c.class_ids)
    ...
    co_docencia = sum(1 for c in translation.courses if len(c.teacher_ids) > 1)
    acopladas = sum(1 for c in translation.courses if len(c.class_ids) > 1)
    sin_curso = sum(1 for c in translation.courses if not c.class_ids)
    sin_aula = sum(1 for c in translation.courses if not c.needs_room)
    print(f"  Cursos (tras fusionar co-docencia): {len(translation.courses)}")
    print(f"    - con varios docentes (co-docencia): {co_docencia}")
    print(f"    - con varios cursos (acopladas):     {acopladas}")
    print(f"    - sin curso (excluidos):             {sin_curso}")
    print(f"    - sin aula:                          {sin_aula}")
